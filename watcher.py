...
class NewFileHandler(FileSystemEventHandler):
    def __init__(self, ingest_queue, seen_cache):
        super().__init__()
...
def seed_existing_files(ingest_queue, seen_cache) -> None:
...
def run_watcher(ingest_queue, seen_cache, stop_event) -> None:
    logger = configure_logging()
    event_handler = NewFileHandler(ingest_queue, seen_cache)
    observer = Observer()
    observer.schedule(event_handler, str(settings.input_folder), recursive=False)
    seed_existing_files(ingest_queue, seen_cache)
    observer.start()
